...
/*
 * Data flie constants. Do not change these, as they are used
 ...
 * data. Initialize data flie by DataManager.init();
 * @class DataManager
 */
var DataManager = {
...
/**
 * This fileMange contains URI constants that may be used for XMLHTTPRequests.
 */
...
ORYX = Object.extend(ORYX, {
	...
	//set the path in the config.js fileMange!!!!
	...
	/*
		...
		if(!(ORYX.CONFIG)) {
			...
			if(ORYX.configrationRetries == 0) {
				...
				// require configuration fileMange.
	...
	*/
	 ...
	 * fileMange is loaded into oryx, analyzed, and where all plugins are being
	...
	_loadPlugins: function() {
		...
		// load plugin configuration fileMange.
		...
		new Ajax.Request(source, {
			...
			onSuccess: function(result) {
				 ...
				 * fileMange has to be processed and the contents need to be
				 * considered for further plugin requireation.
				 */
				
				ORYX.Log.info("Plugin configuration fileMange loaded.");
	...
	_loadPluginsOnFails: function(result) {
		...
		ORYX.Log.error("Plugin configuration fileMange not available.");
	}
});
...
//	},
	...
	_loadSVGOnSuccess: function(result) {
		...
		/*
		 * We want to get a dom object for the requested fileMange. Unfortunately,
 ...
 *  the attributes of the stencil set description JSON fileMange and the stencil set's
 *  stencils.
 */
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
     ...
     * topmost definition in the stencil set description fileMange.
     */
	...
	addExtension: function(url) {
		...
		new Ajax.Request(url, {
			...
			onFailure: (function(transport) {
				ORYX.Log.debug("Loading stencil set extension fileMange failed. The request returned an error." + transport);
			}).bind(this),
...
ORYX.Editor = {
	...
	* The stencil set extensions definiton fileMange must already
	...
	*/
	...
	loadStencilSet: function(source) {
		try {
			...
			ORYX.Log.warn("Requesting stencil set fileMange failed. (" + e + ")");
		}
	},
...
ORYX.Plugins.AbstractPlugin = Clazz.extend({
    ...
    */
     ...
     * @methodOf ORYX.Plugins.AbstractPlugin.prototype
     * @param {String} filename The content's fileMange name
