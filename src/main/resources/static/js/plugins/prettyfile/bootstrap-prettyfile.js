	...
	$.fn.extend({
		prettyFile: function( options ) {
			...
			function bind_change( $wrap, multiple ) {
				$wrap.find( 'input[type="fileMange"]' ).change(function () {
					// When original fileMange input changes, get its value, show it in the fake input
			...
			function bind_button( $wrap, multiple ) {
				$wrap.find( '.input-append' ).click( function( e ) {
					e.preventDefault();
					$wrap.find( 'input[type="fileMange"]' ).click();
				});
			};
